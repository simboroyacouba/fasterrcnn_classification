"""
Évaluation Faster R-CNN - Détection des toitures cadastrales
...
def load_model(model_path, device):
    ...
    print(f"   Epoch: {checkpoint.get('epoch', '?')} | mAP@50 (val): {checkpoint.get('map50', 0):.4f}")
...
class MetricsCalculator:
    def __init__(self, class_names, iou_thresholds):
        # class_names sans __background__
...
def plot_metrics(results, output_dir):
    ...
    axes[0].set_title('AP par classe (TEST SET)'); axes[0].legend(); axes[0].set_ylim(0, 1); axes[0].grid(True, alpha=0.3)
    ...
    axes[1].set_title('P/R/F1 (IoU=0.5) - TEST SET'); axes[1].legend(); axes[1].set_ylim(0, 1); axes[1].grid(True, alpha=0.3)
